## Agenda
- Example code to try out zenml and its orchestration capabilities with OSS version.

## Structure
- Step classes
- Pipeline class
- Implementation classes

## How to Run
1. Clone the repository:
   ```sh
   git clone https://github.com/anupmanekar/mlops-with-zenml.git
   cd mlops-with-zenml
   ```

2. Install the required dependencies:
   ```sh
   pip install -r requirements.txt
   ```

3. Run the pipeline:
   ```sh
   python run_pipeline.py
   ```

## Dependencies
The project requires the following dependencies, which are listed in `requirements.txt`:
- aiohappyeyeballs==2.3.6
- aiohttp==3.10.3
- aiosignal==1.3.1
- alembic==1.8.1
- altair==5.4.0
- aniso8601==9.0.1
- annotated-types==0.7.0
- anyio==4.4.0
- asttokens==2.4.1
- attrs==24.2.0
- bcrypt==4.0.1
- blinker==1.8.2
- cachetools==5.4.0
- catboost==1.2.5
- certifi==2024.7.4
- cffi==1.17.0
- charset-normalizer==3.3.2
- click==8.1.3
- click-params==0.3.0
- cloudpickle==2.2.1
- colorlog==6.8.2
- comm==0.2.2
- contourpy==1.2.1
- cryptography==43.0.0
- cycler==0.12.1
- databricks-sdk==0.30.0
- decorator==5.1.1
- Deprecated==1.2.14
- distro==1.9.0
- docker==7.1.0
- entrypoints==0.4
- executing==2.0.1
- fastapi==0.110.0
- Flask==3.0.3
- fonttools==4.53.1
- frozenlist==1.4.1
- gitdb==4.0.11
- GitPython==3.1.43
- google-auth==2.34.0
- graphene==3.3
- graphql-core==3.2.3
- graphql-relay==3.2.0
- graphviz==0.20.3
- gunicorn==22.0.0
- h11==0.14.0
- httplib2==0.19.1
- httptools==0.6.1
- idna==3.7
- importlib_metadata==7.2.1
- ipinfo==5.0.1
- ipython==8.26.0
- ipywidgets==8.1.3
- itsdangerous==2.2.0
- jedi==0.19.1
- Jinja2==3.1.4
- joblib==1.4.2
- jsonschema==4.23.0
- jsonschema-specifications==2023.12.1
- jupyterlab_widgets==3.0.11
- kiwisolver==1.4.5
- lightgbm @ file:///private/var/folders/k1/30mswbxs7r1g6zwn8y4fyt500000gp/T/abs_0e15gqmeju/croot/lightgbm_1723565410808/work
- Mako==1.3.5
- Markdown==3.7
- markdown-it-py==3.0.0
- MarkupSafe==2.1.5
- matplotlib==3.9.2
- matplotlib-inline==0.1.7
- mdurl==0.1.2
- mlflow==2.15.1
- mlflow-skinny==2.15.1
- multidict==6.0.5
- narwhals==1.4.2
- numpy @ file:///private/var/folders/k1/30mswbxs7r1g6zwn8y4fyt500000gp/T/abs_a51i_mbs7m/croot/numpy_and_numpy_base_1708638620867/work/dist/numpy-1.26.4-cp311-cp311-macosx_11_0_arm64.whl#sha256=3d90dd3382cff7becb2384f73058a8e72b81c697e8bb77f1c69a82caca5b0c57
- opentelemetry-api==1.26.0
- opentelemetry-sdk==1.26.0
- opentelemetry-semantic-conventions==0.47b0
- optuna==3.6.1
- orjson==3.10.7
- packaging==24.1
- pandas==2.2.2
- parso==0.8.4
- passlib==1.7.4
- pexpect==4.9.0
- pillow==10.4.0
- plotly==5.23.0
- prompt_toolkit==3.0.47
- protobuf==5.27.3
- psutil==6.0.0
- ptyprocess==0.7.0
- pure_eval==0.2.3
- pyarrow==15.0.2
- pyasn1==0.6.0
- pyasn1_modules==0.4.0
- pycparser==2.22
- pydantic==2.7.4
- pydantic-settings==2.2.1
- pydantic_core==2.18.4
- pydeck==0.9.1
- Pygments==2.18.0
- PyJWT==2.7.0
- PyMySQL==1.1.1
- pyparsing==2.4.7
- python-dateutil==2.9.0.post0
- python-dotenv==1.0.1
- python-multipart==0.0.9
- pytz==2024.1
- PyYAML==6.0.2
- querystring-parser==1.2.4
- referencing==0.35.1
- requests==2.32.3
- rich==13.7.1
- rpds-py==0.20.0
- rsa==4.9
- scikit-learn==1.5.1
- scipy @ file:///private/var/folders/nz/j6p8yfhx1mv_0grj5xl4650h0000gp/T/abs_b9t0odioph/croot/scipy_1717521487325/work/dist/scipy-1.13.1-cp311-cp311-macosx_11_0_arm64.whl#sha256=63d35c85356bd6a7d204bc184b0cd2a0220b8c9faf06c4f5de5a4e6bc73325f3
- secure==0.3.0
- six==1.16.0
- smmap==5.0.1
- sniffio==1.3.1
- SQLAlchemy==2.0.32
- SQLAlchemy-Utils==0.41.2
- sqlmodel==0.0.18
- sqlparse==0.5.1
- stack-data==0.6.3
- starlette==0.36.3
- streamlit==1.37.1
- tenacity==8.5.0
- threadpoolctl==3.5.0
- toml==0.10.2
- tornado==6.4.1
- tqdm==4.66.5
- traitlets==5.14.3
- typing_extensions @ file:///home/conda/feedstock_root/build_artifacts/typing_extensions_1717802530399/work
- tzdata==2024.1
- urllib3==2.2.2
- uvicorn==0.30.6
- uvloop==0.20.0
- validators==0.18.2
- watchfiles==0.23.0
- wcwidth==0.2.13
- websockets==12.0
- Werkzeug==3.0.3
- widgetsnbextension==4.0.11
- wrapt==1.16.0
- xgboost==2.1.1
- yarl==1.9.4
- zenml==0.64.0
- zipp==3.20.0

## Project Structure
The project is organized as follows:
- `data/`: Contains the dataset used for training and evaluation.
- `model/`: Contains the implementation of data preprocessing, model training, and evaluation strategies.
- `pipelines/`: Contains the pipeline definition.
- `steps/`: Contains the steps used in the pipeline.
- `run_pipeline.py`: Script to run the pipeline.
- `requirements.txt`: List of dependencies required for the project.
